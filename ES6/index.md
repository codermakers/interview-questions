...
````HTTP
【概念及作用】 Promise是ES6引入的一种异步编程的解决方案，旨在解决传统回调函数导致的 `回调地狱`问题，使异步代码更易读、可维护。
【特点】
  ...
  // 竞态请求（如超时控制）
  Promise.race([fetchData(), timeout(5000)]).then((data) => handleData(data));
...
- 4.3、手写PromiseA+、 Promise.all、Promise.race、Promise.finally实现
...
- forEach升级版的for、侧重于遍历
- map生成新数组，不影响原数组，支持链式调用，侧重转换数据
- 两者均无法通过 break 或 return 提前终止遍历（会抛出语法错误）。若需提前终止，需改用 for 循环或 some/every 方法。如果forEach 强行终止，可通过throw error`抛出异常`终止循环
