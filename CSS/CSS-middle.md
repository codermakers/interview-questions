...
- 盒模型的两种模式
  ...
  - IE 盒模型(怪异盒模型)(border-box): width 和 height 是内容区、内边距、边框的总和
    ...
    > PS: CSS3 中 box-sizing 属性可决定网页选择何种盒模型进行渲染页面,默认content-box(开发中优先使用border-box,避免padding/border改变元素宽度，减少布局计算)

#### 2、CSS 三大特性

- 层叠性: CSS解决样式冲突的一种能力
- 优先级(特异性)：
  - 多个选择器作用于同一个元素，浏览器会根据CSS优先级决定最终样式渲染
  - CSS优先级则根据CSS声明的权重为准，而CSS权重的计算有一套自己的规则，取决于各类选择器的数量(a,b,c,d)。
  - CSS权重计算规则
    - 相同权重，遵循`就近原则`,即后面的覆盖前面的。
    - 权重规律: !important权重最高，其次按照 (内联样式 ->1000),id （100）,class/属性选择器/伪类选择器之和(10), 标签选择器/伪元素选择器之和(1))，继承来得样式和通配符之类的都是0.
    - 比较的时候，从左到右，依此比较，高位比过，低位无需再比，以高位的数量优先，比如2个权重分别是（0,0,2,1）和(0,0,1,100)，此时前者的权重高。不存在`进位`现象
    - 内部嵌入的style和link引入的外部样式权重相同，取决于引入顺序。
    - 通用选择器（\*），组合符号（+，>，~，“ ”，||）和否定伪类（:not()）对优先级没有影响。（但是，在在 :not() 内部声明的选择器会影响优先级）
    - !important声明的样式权重最高，开发过程中非必要不使用。

- 继承性: 某些样式可以从父元素可以继承给子元素。
  - 可继承属性：多为「文本 / 字体 / 颜色 / 排版」相关，如font-、text-、line-height等
  - 不可继承属性：多为「布局 / 盒模型 / 定位 / 显示」相关（如width、margin、position、background-color）
  - 例外情况：某些标签因为浏览器默认样式，导致视觉上不继承，开发者可通过重置样式覆盖，或者inherit属性强制继承即可
    - a标签不继承color
    - h1-h6不继承font-size
    - button/input 等表单元素不继承font-family（字体）等等
...
- Grid 布局
  ...
  .parent {
    display: grid;
    place-items: center; // align-items: center; jusify-items: center;
  }
  ```
...
- absolute(绝对定位)
  - 特征
    ...
    - 脱离文档流，不占位原位置空间，注意此时元素的`display: block`,但是表现为 `inline-block` 特性,默认值 100%失效,需要显式设置
  - 应用场景：
    - 常配合父元素 relative 布局，
- fixed(固定定位)
  - 特征
    - 相对于浏览器的视口(viewport)(`不绝对`)进行定位； 注意：当元素祖先的 transform、perspective、filter 或 backdrop-filter 属性非 none 时，容器由视口改为该祖先
    - 元素脱离文档流，不占位原位置空间,注意此时元素的`display: block`,但是表现为 `inline-block` 特性,默认值 100%失效,需要显式设置
  - 应用场景
    - 固定导航栏、悬浮按钮、广告栏、通栏等
  - 注意事项：
...
- 通过position定位或者 marign 将元素移出视口

#### 7、 CSS 权重计算方式

!important > 内联样式> id > class/属性选择器/伪类选择器 > 标签选择器/伪元素选择器 > 通配符/继承来得样式/...
...
- 概念: 元素布局时的一个参考框，用于元素尺寸(百分比)和定位偏移的计算。
- 确定一个元素的包含块的条件：依据于该元素的 position 属性
  - 如果是 relative/static/sticky，包含块就是最近的块级祖先元素的内容区
...
- 扇形实现
  ...
  <!-- 圆锥渐变 conic-gradient -- > .sector-conic {
    width: 200px;
    height: 200px;
    border-radius: 50%;
